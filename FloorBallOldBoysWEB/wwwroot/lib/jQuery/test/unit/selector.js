...
QUnit.test("element",
	function(assert) {
		...
		assert.deepEqual(jQuery("p", fixture).get(),
			q("firstp", "ap", "sndp", "en", "sap", "first"),
			"Finding elements with a Node context.");
		assert.deepEqual(jQuery("p", "#qunit-fixture").get(),
			q("firstp", "ap", "sndp", "en", "sap", "first"),
			"Finding elements with a selector context.");
		assert.deepEqual(jQuery("p", jQuery("#qunit-fixture")).get(),
			q("firstp", "ap", "sndp", "en", "sap", "first"),
			"Finding elements with a jQuery object context.");
		assert.deepEqual(jQuery("#qunit-fixture").find("p").get(),
			q("firstp", "ap", "sndp", "en", "sap", "first"),
		...
		// #7533
		assert.equal(jQuery("<div id=\"A'B~C.D[E]\"><p>foo</p></div>").find("p").length,
			1,
...
QUnit.test("name",
	function(assert) {
		...
		assert.equal(jQuery("input", form[0]).length,
			1,
...
QUnit.test("selectors with comma",
	function(assert) {
		assert.expect(4);
...
QUnit.test("child and adjacent",
	function(assert) {
		...
		if (jQuery.find.compile) {
			...
			assert.t("Combinators are not skipped when mixing general and specific",
		...
		assert.t("Multiple combinators selects all levels",
			"#siblingTest em *",
			["siblingchild", "siblinggrandchild", "siblinggreatgrandchild"]);
		assert.t("Multiple combinators selects all levels",
			"#siblingTest > em *",
			["siblingchild", "siblinggrandchild", "siblinggreatgrandchild"]);
		assert.t("Multiple sibling combinators doesn't miss general siblings",
...
QUnit.test("attributes",
	function(assert) {
		...
		assert.t("Find elements with a tabindex attribute",
			"[tabindex]",
			[
				"listWithTabIndex", "foodWithNegativeTabIndex", "linkWithTabIndex", "linkWithNegativeTabIndex",
				"linkWithNoHrefWithTabIndex", "linkWithNoHrefWithNegativeTabIndex"
			]);
		...
		assert.t("Attribute Equals", "#qunit-fixture a[ rel = 'bookmark' ]", ["simon1"]);
		assert.t("Attribute Equals Number",
		...
		assert.t("Multiple Attribute Equals",
...
QUnit[jQuery.find.compile ? "test" : "skip"]("disconnected nodes",
	function(assert) {
		assert.expect(3);
...
testIframe(
	...
	function(assert, jQuery, window, document) {
		...
		jQuery.expandedEach(["draggable", "contenteditable", "aria-disabled"],
			function(i, val) {
...
testIframe(
	...
	function(assert, jQuery, window, document) {
		...
		assert.deepEqual($cached(".test a").get(),
...
QUnit.asyncTest("Iframe dispatch should not affect jQuery (#13936)",
	1,
	function(assert) {
		var loaded = false,
			thrown = false,
...
QUnit.test("Ensure escapeSelector exists (escape tests in Sizzle)",
	function(assert) {
		assert.expect(1);
