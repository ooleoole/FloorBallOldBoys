...
*/
		...
		assert.strictEqual(elem[0].ownerDocument,
			document,
			"Empty attributes object is not interpreted as a document (trac-8950)");
	});

QUnit[jQuery.find.compile ? "test" : "skip"]("jQuery(selector, context)",
	function(assert) {
		assert.expect(3);
		assert.deepEqual(jQuery("div p", "#qunit-fixture").get(),
			q("sndp", "en", "sap"),
			"Basic selector with string as context");
		assert.deepEqual(jQuery("div p", q("qunit-fixture")[0]).get(),
			q("sndp", "en", "sap"),
			"Basic selector with element as context");
		assert.deepEqual(jQuery("div p", jQuery("#qunit-fixture")).get(),
			q("sndp", "en", "sap"),
			"Basic selector with jQuery object as context");
	});

QUnit.test("globalEval",
	function(assert) {
		assert.expect(3);
...
QUnit.test("globalEval with 'use strict'",
	function(assert) {
		assert.expect(1);
...
QUnit.test("globalEval execution after script injection (#7862)",
	function(assert) {
		assert.expect(1);
...
QUnit[typeof Symbol === "function" ? "test" : "skip"]("isPlainObject(Symbol)",
	function(assert) {
		assert.expect(2);
...
QUnit.test("isPlainObject(localStorage)",
	function(assert) {
		assert.expect(1);
...
QUnit[typeof Symbol === "function" ? "test" : "skip"]("isNumeric(Symbol)",
	function(assert) {
		assert.expect(2);
...
QUnit.test("XSS via location.hash",
	function(assert) {
		assert.expect(1);
...
QUnit.test("jQuery('html')",
	function(assert) {
		...
		div = jQuery("<div> some text<p>some p</p> more text</div>")[0];
		...
		assert.equal(jQuery("element[attribute='<div></div>']").length,
			0,
		...
		if (jQuery.find.compile) {
			assert.equal(jQuery("element:not(<div></div>)").length,
				0,
...
QUnit.test("jQuery(element with non-alphanumeric name)",
	function(assert) {
		assert.expect(36);
...
QUnit.test("jQuery('massive html #7990')",
	function(assert) {
		assert.expect(3);
...
QUnit.test("jQuery('html', context)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("jQuery(selector, xml).text(str) - loaded via xml document",
	function(assert) {
		assert.expect(2);
...
QUnit.test("get()",
	function(assert) {
		assert.expect(1);
		assert.deepEqual(jQuery("#qunit-fixture p").get(),
...
QUnit.test("get(-Number)",
	function(assert) {
		...
		assert.strictEqual(jQuery("#firstp").get(-2),
			undefined,
...
QUnit.test("jQuery.map",
	function(assert) {
		assert.expect(25);

		var i, label, result, callback;

		result = jQuery.map([3, 4, 5],
			function(v, k) {
				return k;
			});
...
QUnit.test("jQuery.grep(Array-like)",
	function(assert) {
		assert.expect(7);
...
QUnit.test("jQuery.extend(Object, Object)",
	function(assert) {
		assert.expect(28);
		...
		assert.ok(jQuery.isPlainObject(jQuery.extend(true, { "arr": arr }, { "arr": {} })["arr"]),
...
QUnit.test("jQuery.extend(Object, Object {created with \"defineProperties\"})",
	function(assert) {
		assert.expect(2);
...
QUnit.test("jQuery.extend(true,{},{a:[], o:{}}); deep copy with array, followed by object",
	function(assert) {
		assert.expect(2);
...
QUnit.test("jQuery.each(Object,Function)",
	function(assert) {
		assert.expect(23);
		...
		seen = {};
		jQuery.each({ name: "name", lang: "lang" },
			function(k, v) {
				seen[k] = v;
			});
...
QUnit.test("jQuery.each/map(undefined/null,Function)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("JIT compilation does not interfere with length retrieval (gh-2145)",
	function(assert) {
		assert.expect(4);
...
QUnit.test("jQuery.makeArray",
	function(assert) {
		assert.expect(15);

		assert.equal(jQuery.makeArray(jQuery("html>*"))[0].nodeName.toUpperCase(), "HEAD", "Pass makeArray a jQuery object");

		assert.equal(jQuery.makeArray(document.getElementsByName("PWD")).slice(0, 1)[0].name,
			"PWD",
			"Pass makeArray a nodelist");

		assert.equal((function() { return jQuery.makeArray(arguments); })(1, 2).join(""),
			"12",
		...
		assert.equal(jQuery.makeArray(document.createElement("div"))[0].nodeName.toUpperCase(),
			"DIV",
			"Pass makeArray a single node");

		assert.equal(jQuery.makeArray({ length: 2, 0: "a", 1: "b" }).join(""),
			"ab",
...
QUnit.test("jQuery.inArray",
	function(assert) {
		...
		assert.equal(jQuery.inArray(0, undefined),
			-1,
			"Search in 'undefined' as array returns -1 and doesn't throw exception");
	});

QUnit.test("jQuery.isEmptyObject",
	function(assert) {
		assert.expect(2);
...
QUnit.test("jQuery.parseHTML",
	function(assert) {
		...
		assert.equal(jQuery.parseHTML("<td><td>")[1].parentNode.nodeType,
			11,
...
QUnit.test("jQuery.parseHTML(<a href>) - gh-2965",
	function(assert) {
		assert.expect(1);
...
if (jQuery.support.createHTMLDocument) {
	QUnit.asyncTest("jQuery.parseHTML",
		function(assert) {
			assert.expect(1);
...
QUnit.test("Iterability of jQuery objects (gh-1693)",
	function(assert) {
		assert.expect(1);
...
QUnit[jQuery.Deferred ? "test" : "skip"]("jQuery.readyException (original)",
	function(assert) {
		assert.expect(1);

		var message;

		this.sandbox.stub(window,
			"setTimeout",
			function(fn) {
				try {
					fn();
...
QUnit[jQuery.Deferred ? "test" : "skip"]("jQuery.readyException (custom)",
	function(assert) {
		assert.expect(1);

		var done = assert.async();

		this.sandbox.stub(jQuery,
			"readyException",
			function(error) {
				assert.strictEqual(
					error.message,
