...
// Can't test what ain't there
	...
	QUnit[jQuery.find.compile ? "test" : "skip"]("sanity check",
		function(assert) {
			assert.expect(1);
			assert.equal(jQuery("#dl:visible, #qunit-fixture:visible, #foo:visible").length,
				3,
				"QUnit state is correct for testing effects");
		});

	QUnit.test("show() basic",
		function(assert) {
			assert.expect(1);
	...
	supportjQuery.each(hideOptions,
		function(type, setup) {
			QUnit.test("show(Number) - " + type + " hidden",
				function(assert) {
					assert.expect(30);
					...
					jQuery.each(test,
						function(selector, expected) {
							jQuery(selector, "#show-tests").each(function() {
								assert.equal(jQuery(this).css("display"),
									expected,
...
// Supports #7397
	supportjQuery.each(hideOptions,
		function(type, setup) {
			QUnit.test("Persist correct display value - " + type + " hidden",
				function(assert) {
					assert.expect(3);
	...
	QUnit.test("animate(Hash, Object, Function)",
		function(assert) {
			assert.expect(1);
			var hash = { opacity: "show" },
				hashCopy = jQuery.extend({}, hash);
			jQuery("#foo").animate(hash,
				0,
				function() {
	...
	QUnit.test("animate relative values",
		function(assert) {
			...
			jQuery.each(bases,
				function(_, baseUnit) {
					jQuery.each(adjustments,
						function(_, adjustUnit) {
							...
							elem.css("width", base).animate(adjust,
								100,
								function() {
									assert.equal(this.offsetHeight,
	...
	QUnit.test("animate negative height",
		function(assert) {
			assert.expect(1);
			jQuery("#foo").animate({ height: -100 },
				100,
				function() {
	...
	QUnit.test("animate negative margin",
		function(assert) {
			assert.expect(1);
			jQuery("#foo").animate({ "marginTop": -100 },
				100,
				function() {
	...
	QUnit.test("animate negative margin with px",
		function(assert) {
			assert.expect(1);
			jQuery("#foo").animate({ marginTop: "-100px" },
				100,
				function() {
					assert.equal(jQuery(this).css("marginTop"), "-100px", "Verify margin.");
				});
	...
	QUnit.test("animate negative padding",
		function(assert) {
			assert.expect(1);
			jQuery("#foo").animate({ "paddingBottom": -100 },
				100,
				function() {
	...
	QUnit.test("animate block as inline width/height",
		function(assert) {
			assert.expect(3);

			jQuery("#foo").css({ display: "inline", width: "", height: "" }).animate({ width: 42, height: 42 },
				100,
				function() {
					assert.equal(jQuery(this).css("display"),
						"inline-block",
	...
	QUnit.test("animate native inline width/height",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("animate block width/height",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("animate table width/height",
		function(assert) {
			assert.expect(1);

			jQuery("#table").animate({ width: 42, height: 42 },
				100,
				function() {
	...
	QUnit.test("animate table-row width/height",
		function(assert) {
			assert.expect(3);
			var tr = jQuery("#table")
	...
	QUnit.test("animate table-cell width/height",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("animate percentage(%) on width/height",
		function(assert) {
			assert.expect(2);
			...
			$div.animate({ width: "25%", height: "25%" },
				13,
				function() {
	...
	QUnit.test("animate resets overflow-x and overflow-y when finished",
		function(assert) {
			assert.expect(2);
			jQuery("#foo")
...
*/
	...
	QUnit.test("animate option { queue: false }",
		function(assert) {
			assert.expect(2);
			var foo = jQuery("#foo");
	...
	QUnit.test("animate option { queue: true }",
		function(assert) {
			assert.expect(2);
			var foo = jQuery("#foo");
	...
	QUnit.test("animate option { queue: 'name' }",
		function(assert) {
			assert.expect(5);
			var foo = jQuery("#foo"),
	...
	QUnit.test("animate with no properties",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("animate hyphenated properties",
		function(assert) {
			assert.expect(1);

			jQuery("#foo")
				.css("font-size", 10)
				.animate({ "font-size": 20 },
					200,
					function() {
	...
	QUnit.test("animate non-element",
		function(assert) {
			assert.expect(1);

			var obj = { test: 0 };

			jQuery(obj).animate({ test: 200 },
				200,
				function() {
...
// In IE9 inside testswarm this test doesn't work properly
	(function() {
		...
		QUnit[type]("stop() - several in queue",
			function(assert) {
				assert.expect(5);
	...
	QUnit.test("stop(clearQueue, gotoEnd)",
		function(assert) {
			assert.expect(1);
	...
	QUnit.test("stop( queue, ..., ... ) - Stop single queues",
		function(assert) {
			assert.expect(3);
			var saved,
	...
	QUnit[jQuery.find.compile ? "test" : "skip"]("toggle()",
		function(assert) {
			assert.expect(6);
			var x = jQuery("#foo");
	...
	QUnit.test("jQuery.fx.prototype.cur() - <1.8 Back Compat",
		function(assert) {
			assert.expect(7);
	...
	QUnit.test("Overflow and Display",
		function(assert) {
			assert.expect(4);
					...
					QUnit.test(fn + " to " + tn,
						function(assert) {
							...
							jQuery.when(elem).done(function($elem) {
								...
								if (t_w === "show") {
									assert.equal($elem.css("display"),
										"block",
										"Showing, display should block: " + elem.style.display);
								}

								if (t_w === "hide" || t_w === "show") {
									assert.ok(f_w === "" ? elem.style.width === f_w : elem.style.width.indexOf(f_w) === 0,
										"Width must be reset to " + f_w + ": " + elem.style.width);
								}

								if (t_h === "hide" || t_h === "show") {
									assert.ok(f_h === "" ? elem.style.height === f_h : elem.style.height.indexOf(f_h) === 0,
								...
								if (t_o.constructor === Number) {
									...
									assert.ok(jQuery.css(elem, "opacity") !== "" || cur_o === t_o,
								...
								if (t_w.constructor === Number) {
									...
									assert.ok(elem.style.width !== "" || cur_w === t_w,
								...
								if (t_h.constructor === Number) {
									...
									assert.ok(elem.style.height !== "" || cur_h === t_h,
	...
	QUnit.test("jQuery.show('fast') doesn't clear radio buttons (bug #1095)",
		function(assert) {
			assert.expect(4);
	...
	QUnit.test("interrupt toggle",
		function(assert) {
			...
			jQuery.each({ slideToggle: "height", fadeToggle: "opacity", toggle: "width" },
				function(method, prop) {
					...
					setTimeout(function() {
							$methodElems.stop().each(function() {
								assert.notEqual(jQuery(this).css(prop),
	...
	QUnit.test("animate with per-property easing",
		function(assert) {
	...
	QUnit.test("animate with CSS shorthand properties",
		function(assert) {
			assert.expect(11);
	...
	QUnit.test("hide hidden elements, with animation (bug #7141)",
		function(assert) {
			assert.expect(4);
	...
	QUnit.test("animate unit-less properties (#4966)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("animate properties missing px w/ opacity as last (#9074)",
		function(assert) {
			assert.expect(6);
	...
	QUnit.test("callbacks should fire in correct order (#9100)",
		function(assert) {
			assert.expect(1);
	...
	QUnit.test("callbacks that throw exceptions will be removed (#5684)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("animate will scale margin properties individually",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("Do not append px to 'fill-opacity' #9548",
		function(assert) {
			assert.expect(1);

			var $div = jQuery("<div>").appendTo("#qunit-fixture");

			$div.css("fill-opacity", 0).animate({ "fill-opacity": 1.0 },
				0,
				function() {
	...
	QUnit.test("line-height animates correctly (#13855)",
		function(assert) {
			assert.expect(12);
			...
			var t0,
				...
				initialHeight = jQuery.map(animated,
					function(el) {
...
// Start 1.8 Animation tests
	QUnit.test("jQuery.Animation( object, props, opts )",
		function(assert) {
			assert.expect(4);
	...
	QUnit.test("Animate Option: step: function( percent, tween )",
		function(assert) {
			assert.expect(1);
	...
	QUnit.test("Animate callbacks have correct context",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("User supplied callback called after show when fx off (#8892)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("animate should set display for disconnected nodes",
		function(assert) {
			assert.expect(20);
			...
			assert.strictEqual($divInline.show()[0].style.display,
				"inline",
				"show() should not change display if it already set");

			assert.expectJqData(env, $divNone[0], "olddisplay");

			jQuery.each(showMethods,
				function(name, opt) {
					jQuery.fn[name].apply(jQuery("<div/>"),
						opt.concat([
							function() {
								assert.strictEqual(jQuery(this).css("display"),
									nullParentDisplay,
									"." + name + " block with null parentNode");
							}
						]));

					jQuery.fn[name].apply(jQuery("<div>test</div>"),
						opt.concat([
							function() {
								assert.strictEqual(jQuery(this).css("display"),
	...
	QUnit[jQuery.find.compile ? "test" : "skip"]("Initial step callback should show element as :animated (#14623)",
		function(assert) {
			assert.expect(1);
	...
	QUnit.test("hide called on element within hidden parent should set display to none (#10045)",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("hide, fadeOut and slideUp called on element width height and width = 0 should set display to none",
		function(assert) {
			assert.expect(5);
			...
			jQuery.when(
				...
				assert.strictEqual(elems.get(3).style.display,
					"none",
					"fadeOut() called on element width height and width = 0 should set display to none");
				assert.strictEqual(elems.get(4).style.display,
					"none",
	...
	QUnit.test("hide should not leave hidden inline elements visible (#14848)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("Handle queue:false promises",
		function(assert) {
			assert.expect(10);
	...
	QUnit.test("multiple unqueued and promise",
		function(assert) {
			assert.expect(4);
	...
	QUnit.test("animate does not change start value for non-px animation (#7109)",
		function(assert) {
			assert.expect(1);
	...
	QUnit.test("non-px animation handles non-numeric start (#11971)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("Animation callbacks (#11797)",
		function(assert) {
			assert.expect(15);
	...
	QUnit.test("Animation callbacks in order (#2292)",
		function(assert) {
			assert.expect(9);
	...
	QUnit.test("Animate properly sets overflow hidden when animating width/height (#12117)",
		function(assert) {
			assert.expect(8);
	...
	QUnit.test("Each tick of the timer loop uses a fresh time (#12837)",
		function(assert) {
			var lastVal,
				tmp = jQuery({
					test: 0
	...
	QUnit.test("Animations with 0 duration don't ease (#12273)",
		function(assert) {
			assert.expect(1);
	...
	jQuery.map(["toggle", "slideToggle", "fadeToggle"],
		function(method) {

			// this test would look a lot better if we were using something to override
			// the default timers
			var duration = 1500;
			QUnit.test("toggle state tests: " + method + " (#8685)",
				function(assert) {
	...
	QUnit.test("jQuery.fx.start & jQuery.fx.stop hook points",
		function(assert) {
	...
	QUnit.test(".finish() completes all queued animations",
		function(assert) {
			var animations = {
				    top: 100,
				    left: 100,
	...
	QUnit.test(".finish( false ) - unqueued animations",
		function(assert) {
			var animations = {
				    top: 100,
				    left: 100,
	...
	QUnit.test(".finish( \"custom\" ) - custom queue animations",
		function(assert) {
			var animations = {
				    top: 100,
				    left: 100,
	...
	QUnit.test(".finish() calls finish of custom queue functions",
		function(assert) {
	...
	QUnit.test(".finish() is applied correctly when multiple elements were animated (#13937)",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("slideDown() after stop() (#13483)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("Respect display value on inline elements (#14824)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("jQuery.easing._default (gh-2218)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("jQuery.easing._default in Animation (gh-2218",
		function(assert) {
			assert.expect(3);
			...
			animation.done(function() {
				...
				assert.strictEqual(animation.opts.easing,
					"custom",
	...
	QUnit.test("jQuery.easing._default in Tween (gh-2218)",
		function(assert) {
			assert.expect(3);
			...
			assert.ok(called, "Custom jQuery.easing._default called");
			assert.strictEqual(tween.easing,
				"custom",
				"Animation used custom jQuery.easing._default");
			jQuery.easing._default = defaultEasing;
			delete jQuery.easing.custom;
	...
	QUnit.test("Display value is correct for disconnected nodes (trac-13310)",
		function(assert) {
			assert.expect(3);

			var div = jQuery("<div/>");

			assert.equal(div.css("display", "inline").hide().show().appendTo("body").css("display"),
				"inline",
				"Initialized display value has returned");
			div.remove();

			div.css("display", "none").hide();
			assert.equal(jQuery._data(div[0], "olddisplay"),
				undefined,
	...
	QUnit.test("Show/hide/toggle and display: inline",
		function(assert) {
			assert.expect(40);
	...
	QUnit.test("jQuery.speed( speed, easing, complete )",
		function(assert) {
			testEasing(assert,
				10,
				"linear",
	...
	QUnit.test("jQuery.speed( speed, easing, complete ) - with easing function",
		function(assert) {
			testEasing(
				assert,
				10,
	...
	QUnit.test("jQuery.speed( options )",
		function(assert) {
			testEasing(assert,
				{
					duration: 10,
	...
	QUnit.test("jQuery.speed( options ) - with easing function",
		function(assert) {
			testEasing(assert,
				{
					duration: 10,
	...
	QUnit.test("jQuery.speed( options ) - queue values",
		function(assert) {
			assert.expect(5);
	...
	QUnit.test("jQuery.speed() - durations",
		function(assert) {
			assert.expect(5);
