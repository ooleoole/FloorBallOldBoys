(function() {
	...
	QUnit.test("disconnected element",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("hidden (display: none) element",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("hidden (visibility: hidden) element",
		function(assert) {
			assert.expect(3);
		...
		function(assert, $) {
			...
			jQuery.each(tests,
				function() {
					...
					assert.equal($(this.id).offset().top,
						this.top + 1,
						"jQuery('" + this.id + "').offset({ top: " + (this.top + 1) + " })");
					assert.equal($(this.id).offset().left,
						this.left + 1,
	...
	QUnit.test("chaining",
		function(assert) {
			...
			assert.equal(jQuery("#non-existent").offset(coords).jquery,
				jQuery.fn.jquery,
				"offset(coords) with empty jQuery set returns jQuery object");
			assert.equal(jQuery("#absolute-1").offset(undefined).jquery,
				jQuery.fn.jquery,
				"offset(undefined) returns jQuery object (#5571)");
		});

	QUnit.test("offsetParent",
		function(assert) {
			assert.expect(13);
			...
			assert.equal(div.offsetParent()[0],
	...
	QUnit.test("fractions (see #7730 and #7885)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("iframe scrollTop/Left (see gh-1945)",
		function(assert) {
			assert.expect(2);
