...
QUnit.test("jQuery.data & removeData, expected returns",
	function(assert) {
		assert.expect(4);
...
QUnit.test("jQuery._data & _removeData, expected returns",
	function(assert) {
		assert.expect(4);
		var elem = document.body;
...
QUnit.test("jQuery.hasData no side effects",
	function(assert) {
		assert.expect(1);
		var obj = {};
		...
		assert.equal(Object.getOwnPropertyNames(obj).length,
			0,
...
function dataTests(elem, assert) {
	...
	assert.strictEqual(jQuery.data(elem, "foo"),
		"foo1",
	...
	assert.strictEqual(jQuery._data(elem),
		internalDataObj,
	...
	assert.strictEqual(jQuery._data(elem, jQuery.expando),
		undefined,
...
QUnit.test("jQuery.data(document)",
	function(assert) {
		assert.expect(25);
...
QUnit.test("jQuery.data(object/flash)",
	function(assert) {
		assert.expect(25);
...
QUnit.test("jQuery().data() === undefined (#14101)",
	function(assert) {
		assert.expect(2);
...
QUnit.test("jQuery(Element).data(String, Object).data(String)",
	function(assert) {
		assert.expect(18);
		...
		assert.strictEqual(div.data("test", undefined).data("test"),
			"overwritten",
...
QUnit.test("jQuery(plain Object).data(String, Object).data(String)",
	function(assert) {
		assert.expect(16);
...
QUnit.test(".data(object) does not retain references. #13815",
	function(assert) {
		assert.expect(2);
...
QUnit.test("data-* attributes",
	function(assert) {
		...
		assert.strictEqual(child.data("bigassnum"),
...
if (window.JSON && window.JSON.stringify) {
	QUnit.test("JSON serialization (#8108)",
		function(assert) {
			assert.expect(1);
...
QUnit.test(".data should follow html5 specification regarding camel casing",
	function(assert) {
		assert.expect(12);
...
QUnit.test(".data should not miss preset data-* w/ hyphenated property names",
	function(assert) {
...
QUnit.test("jQuery.data should not miss data-* w/ hyphenated property names #14047",
	function(assert) {
...
QUnit.test(".data should not miss attr() set data-* with hyphenated property names",
	function(assert) {
		assert.expect(2);
...
QUnit.test(".data always sets data with the camelCased key (gh-2257)",
	function(assert) {
		assert.expect(18);
...
QUnit.test(".data should not strip more than one hyphen when camelCasing (gh-2070)",
	function(assert) {
		assert.expect(3);
		var div = jQuery("<div data-nested-single='single' data-nested--double='double' data-nested---triple='triple'></div>")
...
QUnit.test(".data supports interoperable removal of hyphenated/camelCase properties",
	function(assert) {
		var div = jQuery("<div/>", { id: "hyphened" }).appendTo("#qunit-fixture"),
			datas = {
				"non-empty": "a string",
...
QUnit.test(".data supports interoperable removal of properties SET TWICE #13850",
	function(assert) {
		var div = jQuery("<div>").appendTo("#qunit-fixture"),
			datas = {
				"non-empty": "a string",
...
QUnit.test(".removeData supports removal of hyphenated properties via array (#12786, gh-2257)",
	function(assert) {
		assert.expect(4);
...
QUnit.test(".removeData should not throw exceptions. (#10080)",
	function(assert) {
		assert.expect(1);
		QUnit.stop();
...
QUnit.test(".data only checks element attributes once. #8909",
	function(assert) {
		assert.expect(2);
		var testing = {
			    "test": "testing",
...
QUnit.test("data-* with JSON value can have newlines",
	function(assert) {
		assert.expect(1);
...
QUnit.test(".data doesn't throw when calling selection is empty. #13551",
	function(assert) {
		assert.expect(1);
...
QUnit.test("acceptData",
	function(assert) {
		...
		assert.strictEqual(jQuery(document.createDocumentFragment()).data("test", 42).data("test"),
			undefined,
			"documentFragment");
...
QUnit.test("Check proper data removal of non-element descendants nodes (#8335)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("Check that the expando is removed when there's no more data",
	function(assert) {
		assert.expect(2);
...
QUnit.test("Check that the expando is removed when there's no more data on non-nodes",
	function(assert) {
		assert.expect(1);
...
QUnit.test(".data(prop) does not create expando",
	function(assert) {
		assert.expect(1);
