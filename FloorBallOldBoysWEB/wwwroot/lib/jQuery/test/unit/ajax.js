...
(function() {
	QUnit.test("Unit Testing Environment",
		function(assert) {
			assert.expect(2);
	...
	function addGlobalEvents(expected, assert) {
		return function() {
			expected = expected || "";
			jQuery(document).on("ajaxStart ajaxStop ajaxSend ajaxComplete ajaxError ajaxSuccess",
				function(e) {
...
//----------- jQuery.ajax()
	...
	ajaxTest("jQuery.ajax() - success callbacks - (url, options) syntax",
		8,
		function(assert) {
			return {
				setup: addGlobalEvents("ajaxStart ajaxStop ajaxSend ajaxComplete ajaxSuccess", assert),
	...
	ajaxTest("jQuery.ajax() - do not execute js (crossOrigin)",
		2,
		function(assert) {
			return {
				create: function(options) {
	...
	ajaxTest("jQuery.ajax() - success callbacks (late binding)",
		8,
		function(assert) {
			return {
				setup: addGlobalEvents("ajaxStart ajaxStop ajaxSend ajaxComplete ajaxSuccess", assert),
	...
	ajaxTest("jQuery.ajax() - success callbacks (oncomplete binding)",
		8,
		function(assert) {
			return {
				setup: addGlobalEvents("ajaxStart ajaxStop ajaxSend ajaxComplete ajaxSuccess", assert),
	...
	ajaxTest("jQuery.ajax() - error callbacks",
		8,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - textStatus and errorThrown values",
		4,
		function(assert) {
			return [
				{
	...
	ajaxTest("jQuery.ajax() - responseText on error",
		1,
		function(assert) {
			return {
	...
	QUnit.asyncTest("jQuery.ajax() - retry with jQuery.ajax( this )",
		2,
		function(assert) {
			var previousUrl,
				firstTime = true;
	...
	ajaxTest("jQuery.ajax() - Accept header",
		1,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - contentType",
		2,
		function(assert) {
			return [
				{
	...
	ajaxTest("jQuery.ajax() - protocol-less urls",
		1,
		function(assert) {
			return {
				url: "//somedomain.com",
	...
	ajaxTest("jQuery.ajax() - traditional param encoding",
		4,
		function(assert) {
			return [
				{
	...
	ajaxTest("jQuery.ajax() - cross-domain detection",
		8,
		function(assert) {
	...
	ajaxTest("jQuery.ajax() - native abort",
		2,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - events with context",
		12,
		function(assert) {
	...
	ajaxTest("jQuery.ajax() - events without context",
		3,
		function(assert) {
	...
	ajaxTest("#15118 - jQuery.ajax() - function without jQuery.event",
		1,
		function(assert) {
			var holder;
			return {
	...
	ajaxTest("#15160 - jQuery.ajax() - request manually aborted in ajaxSend",
		3,
		function(assert) {
			return {
				setup: function() {
					...
					jQuery(document).on("ajaxError ajaxComplete",
						function(e, jqXHR) {
							assert.equal(jqXHR.statusText,
								"abort",
	...
	ajaxTest("jQuery.ajax() - context modification",
		1,
		function(assert) {
			return {
				url: url("data/name.html"),
	...
	ajaxTest("jQuery.ajax() - context modification through ajaxSetup",
		3,
		function(assert) {
			var obj = {};
			return {
	...
	ajaxTest("jQuery.ajax() - disabled globals",
		3,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - xml: non-namespace elements inside namespaced elements",
		3,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - xml: non-namespace elements inside namespaced elements (over JSONP)",
		3,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - HEAD requests",
		2,
		function(assert) {
			return [
				{
	...
	ajaxTest("jQuery.ajax() - beforeSend",
		1,
		function(assert) {
			return {
				url: url("data/name.html"),
	...
	ajaxTest("jQuery.ajax() - beforeSend, cancel request manually",
		2,
		function(assert) {
			return {
				create: function() {
	...
	ajaxTest("jQuery.ajax() - dataType html",
		5,
		function(assert) {
			return {
				setup: function() {
	...
	ajaxTest("jQuery.ajax() - synchronous request",
		1,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - synchronous request with callbacks",
		2,
		function(assert) {
			return {
				url: url("data/json_obj.js"),
	...
	QUnit.asyncTest("jQuery.ajax(), jQuery.get[Script|JSON](), jQuery.post(), pass-through request object",
		8,
		function(assert) {
			...
			jQuery(document).on("ajaxError.passthru",
				function(e, xml) {
					errorCount++;
					errorEx += ": " + xml.status;
				});
			jQuery(document).one("ajaxStop",
				function() {
	...
	jQuery.each([" - Same Domain", " - Cross Domain"],
		function(crossDomain, label) {

			ajaxTest("jQuery.ajax() - JSONP - Query String (?n)" + label,
				4,
				function(assert) {
					return [
						{
			...
			ajaxTest("jQuery.ajax() - JSONP - Explicit callback param" + label,
				10,
				function(assert) {
					return {
						setup: function() {
						...
						requests: [
							{
								...
								success: function(data) {
									...
									assert.strictEqual(window["functionToCleanUp"],
										true,
								...
								crossDomain: crossDomain,
								beforeSend: function() {
									assert.ok(/^data\/jsonp.php\?callback=XXX&_=\d+$/.test(this.url),
			...
			ajaxTest("jQuery.ajax() - JSONP - Callback in data" + label,
				2,
				function(assert) {
					return [
						{
			...
			ajaxTest("jQuery.ajax() - JSONP - POST" + label,
				3,
				function(assert) {
					return [
						{
			...
			ajaxTest("jQuery.ajax() - JSONP" + label,
				3,
				function(assert) {
					return [
						{
	...
	ajaxTest("jQuery.ajax() - script, Remote",
		2,
		function(assert) {
			return {
				setup: function() {
	...
	ajaxTest("jQuery.ajax() - script, Remote with POST",
		3,
		function(assert) {
			return {
				setup: function() {
	...
	ajaxTest("jQuery.ajax() - script, Remote with scheme-less URL",
		2,
		function(assert) {
			return {
				setup: function() {
	...
	ajaxTest("jQuery.ajax() - malformed JSON",
		2,
		function(assert) {
			return {
				url: "data/badjson.js",
	...
	ajaxTest("jQuery.ajax() - script by content-type",
		2,
		function() {
			return [
				{
	...
	ajaxTest("jQuery.ajax() - JSON by content-type",
		5,
		function(assert) {
			return {
				url: "data/json.php",
	...
	ajaxTest("jQuery.ajax() - JSON by content-type disabled with options",
		6,
		function(assert) {
			return {
				url: url("data/json.php"),
	...
	ajaxTest("jQuery.ajax() - simple get",
		1,
		function(assert) {
			return {
				type: "GET",
	...
	ajaxTest("jQuery.ajax() - simple post",
		1,
		function(assert) {
			return {
				type: "POST",
	...
	ajaxTest("jQuery.ajax() - data option - empty bodies for non-GET requests",
		1,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - data - x-www-form-urlencoded (gh-2658)",
		1,
		function(assert) {
			return {
				url: "bogus.html",
	...
	ajaxTest("jQuery.ajax() - data - text/plain (gh-2658)",
		1,
		function(assert) {
			return {
				url: "bogus.html",
	...
	ajaxTest("jQuery.ajax() - data - no processing ",
		1,
		function(assert) {
			return {
				url: "bogus.html",
		...
		function(label, cache) {
			jQuery.each(
				...
				function(type, url) {
					...
					QUnit.asyncTest("jQuery.ajax() - " + type + " support" + label,
						4,
						function(assert) {
							jQuery.ajax({
								url: url,
	...
	ajaxTest("jQuery.ajax() - failing cross-domain (non-existing)",
		1,
		function(assert) {
			return {
	...
	ajaxTest("jQuery.ajax() - failing cross-domain",
		1,
		function(assert) {
			return {
	...
	QUnit.asyncTest("jQuery.ajax() - statusText",
		3,
		function(assert) {
			jQuery.ajax(url("data/statusText.php?status=200&text=Hello")).done(function(_, statusText, jqXHR) {
				...
				assert.ok(jqXHR.statusText === "Hello" || jqXHR.statusText === "OK",
	...
	QUnit.asyncTest("jQuery.ajax() - statusCode",
		20,
		function(assert) {
	...
	ajaxTest("jQuery.ajax() - transitive conversions",
		8,
		function(assert) {
			return [
				{
	...
	ajaxTest("jQuery.ajax() - overrideMimeType",
		2,
		function(assert) {
			return [
				{
	...
	ajaxTest("jQuery.ajax() - empty json gets to error callback instead of success callback.",
		1,
		function(assert) {
			return {
	...
	ajaxTest("#2688 - jQuery.ajax() - beforeSend, cancel request",
		2,
		function(assert) {
			return {
				create: function() {
	...
	ajaxTest("#2806 - jQuery.ajax() - data option - evaluate function values",
		1,
		function(assert) {
			return {
	...
	QUnit.test("#7531 - jQuery.ajax() - Location object as url",
		function(assert) {
			assert.expect(1);
	...
	jQuery.each([" - Same Domain", " - Cross Domain"],
		function(crossDomain, label) {
			ajaxTest("#7578 - jQuery.ajax() - JSONP - default for cache option" + label,
				1,
				function(assert) {
					return {
						url: "data/jsonp.php",
	...
	ajaxTest("#8107 - jQuery.ajax() - multiple method signatures introduced in 1.5",
		4,
		function(assert) {
			return [
				{
	...
	jQuery.each([" - Same Domain", " - Cross Domain"],
		function(crossDomain, label) {
			ajaxTest("#8205 - jQuery.ajax() - JSONP - re-use callbacks name" + label,
				4,
				function(assert) {
					return {
						url: "data/jsonp.php",
	...
	QUnit.test("#9887 - jQuery.ajax() - Context with circular references (#9887)",
		function(assert) {
			assert.expect(2);
	...
	jQuery.each(["as argument", "in settings object"],
		function(inSetting, title) {
			...
			ajaxTest("#10093 - jQuery.ajax() - falsy url " + title,
				4,
				function(assert) {
					return [
	...
	ajaxTest("#11151 - jQuery.ajax() - parse error body",
		2,
		function(assert) {
			return {
	...
	ajaxTest("#11426 - jQuery.ajax() - loading binary data shouldn't throw an exception in IE",
		1,
		function(assert) {
			return {
	...
	if (typeof window.ArrayBuffer === "undefined" || typeof new XMLHttpRequest().responseType !== "string") {
		...
		jQuery.ajaxPrefilter("arraybuffer",
			function(s) {
		...
		ajaxTest("gh-2498 - jQuery.ajax() - binary data shouldn't throw an exception",
			2,
			function(assert) {
				return {
					url: url("data/1x1.jpg"),
	...
	QUnit.asyncTest("#11743 - jQuery.ajax() - script, throws exception",
		1,
		function(assert) {
	...
	jQuery.each(["method", "type"],
		function(_, globalOption) {
	...
	ajaxTest("#13276 - jQuery.ajax() - compatibility between XML documents from ajax requests and parsed string",
		1,
		function(assert) {
			return {
	...
	ajaxTest("#13292 - jQuery.ajax() - converter is bypassed for 204 requests",
		3,
		function(assert) {
			return {
	...
	ajaxTest("#13388 - jQuery.ajax() - responseXML",
		3,
		function(assert) {
			return {
				url: url("data/with_fries.xml"),
	...
	ajaxTest("#13922 - jQuery.ajax() - converter is bypassed for HEAD requests",
		3,
		function(assert) {
			return {
				url: "data/json.php",
	...
	ajaxTest("#14683 - jQuery.ajax() - Exceptions thrown synchronously by xhr.send should be caught",
		4,
		function(assert) {
			return [
				{
	...
	ajaxTest("gh-2587 - when content-type not xml, but looks like one",
		1,
		function(assert) {
			return {
	...
	ajaxTest("gh-2587 - when content-type not json, but looks like one",
		1,
		function(assert) {
			return {
				url: url("data/ajax/content-type.php"),
	...
	ajaxTest("gh-2587 - when content-type not html, but looks like one",
		1,
		function(assert) {
			return {
				url: url("data/ajax/content-type.php"),
	...
	ajaxTest("gh-2587 - when content-type not javascript, but looks like one",
		1,
		function(assert) {
			return {
				url: url("data/ajax/content-type.php"),
	...
	ajaxTest("gh-2587 - when content-type not ecmascript, but looks like one",
		1,
		function(assert) {
			return {
				url: url("data/ajax/content-type.php"),
...
//----------- jQuery.ajaxPrefilter()
	...
	ajaxTest("jQuery.ajaxPrefilter() - abort",
		1,
		function(assert) {
			return {
				dataType: "prefix",
				setup: function() {
					...
					jQuery.ajaxPrefilter("+prefix",
						function(options, _, jqXHR) {
...
//----------- jQuery.ajaxSetup()
	...
	QUnit.asyncTest("jQuery.ajaxSetup()",
		1,
		function(assert) {
			jQuery.ajaxSetup({
				url: url("data/name.php?name=foo"),
	...
	QUnit.asyncTest("jQuery.ajaxSetup({ timeout: Number }) - with global timeout",
		2,
		function(assert) {
			var passed = 0,
				pass = function() {
	...
	QUnit.asyncTest("jQuery.ajaxSetup({ timeout: Number }) with localtimeout",
		1,
		function(assert) {
			jQuery.ajaxSetup({
				timeout: 50
...
//----------- jQuery.domManip()
	...
	QUnit.test("#11264 - jQuery.domManip() - no side effect because of ajaxSetup or global events",
		function(assert) {
			assert.expect(1);
			...
			jQuery(document).on("ajaxStart ajaxStop",
				function() {
	...
	QUnit.test(
		...
		function(assert) {
			...
			jQuery("#qunit-fixture").load("data/ajax/method.php",
				function(method) {
...
//----------- jQuery.get()
	...
	QUnit.asyncTest("jQuery.get( String, Hash, Function ) - parse xml and use text() on nodes",
		2,
		function(assert) {
			jQuery.get(url("data/dashboard.xml"),
				function(xml) {
					var content = [];
	...
	QUnit.asyncTest("#8277 - jQuery.get( String, Function ) - data in ajaxSettings",
		1,
		function(assert) {
			jQuery.ajaxSetup({
				data: "helloworld"
			});
			jQuery.get(url("data/echoQuery.php"),
				function(data) {
...
//----------- jQuery.getJSON()
	...
	QUnit.asyncTest("jQuery.getJSON( String, Hash, Function ) - JSON array",
		5,
		function(assert) {
			jQuery.getJSON(
				url("data/json.php"),
	...
	QUnit.asyncTest("jQuery.getJSON( String, Function ) - JSON object",
		2,
		function(assert) {
			jQuery.getJSON(url("data/json.php"),
				function(json) {
					if (json && json["data"]) {
	...
	QUnit.asyncTest("jQuery.getJSON( String, Function ) - JSON object with absolute url to local content",
		2,
		function(assert) {
			jQuery.getJSON(url(window.location.href.replace(/[^\/]*$/, "") + "data/json.php"),
				function(json) {
					assert.strictEqual(json.data.lang, "en", "Check JSON: lang");
					assert.strictEqual(json.data.length, 25, "Check JSON: length");
					QUnit.start();
...
//----------- jQuery.getScript()
	...
	QUnit.test("jQuery.getScript( String, Function ) - with callback",
		...
		function(assert) {
			...
			Globals.register("testBar");
			jQuery.getScript(url("data/testbar.php"),
				function() {
	...
	QUnit.test("jQuery.getScript( String, Function ) - no callback",
		1,
		function(assert) {
			Globals.register("testBar");
	...
	QUnit.test("#8082 - jQuery.getScript( String, Function ) - source as responseText",
		2,
		function(assert) {
			var done = assert.async();
...
// //----------- jQuery.fn.load()
	...
	QUnit.test("jQuery.fn.load( String )",
		2,
		function(assert) {
			jQuery.ajaxSetup({
				beforeSend: function() {
	...
	QUnit.test("jQuery.fn.load() - 404 error callbacks",
		function(assert) {
			assert.expect(6);
			var done = assert.async();
	...
	QUnit.test("jQuery.fn.load( String, null )",
		2,
		function(assert) {
			jQuery.ajaxSetup({
				beforeSend: function() {
	...
	QUnit.test("jQuery.fn.load( String, undefined )",
		2,
		function(assert) {
			jQuery.ajaxSetup({
				beforeSend: function() {
	...
	// check if load can be called with only url
	QUnit.asyncTest("jQuery.fn.load( URL_SELECTOR )",
		1,
		function(assert) {
			jQuery("#first").load("data/test3.html div.user",
				function() {
	...
	QUnit.asyncTest("jQuery.fn.load( URL_SELECTOR with spaces )",
		1,
		function(assert) {
			jQuery("#first").load("data/test3.html   #superuser ",
				function() {
	...
	QUnit.test("jQuery.fn.load( URL_SELECTOR with non-HTML whitespace(#3003) )",
		function(assert) {
			assert.expect(1);
			var done = assert.async();
			jQuery("#first").load("data/test3.html   #whitespace\\\\xA0 ",
				function() {
					assert.strictEqual(jQuery(this).children("div").length, 1, "Verify that specific elements were injected");
					done();
				});
	...
	QUnit.asyncTest("jQuery.fn.load( String, Function ) - simple: inject text into DOM",
		2,
		function(assert) {
	...
	QUnit.asyncTest("jQuery.fn.load( String, Function ) - check scripts",
		7,
		function(assert) {
			...
			jQuery("#first").load(url("data/test.html"),
				function() {
	...
	QUnit.asyncTest("jQuery.fn.load( String, Function ) - check file with only a script tag",
		3,
		function(assert) {
			Globals.register("testFoo");

			jQuery("#first").load(url("data/test2.html"),
				function() {
					assert.strictEqual(jQuery("#foo").html(), "foo", "Check if script evaluation has modified DOM");
					assert.strictEqual(window["testFoo"], "foo", "Check if script was evaluated after load");
					QUnit.start();
	...
	QUnit.asyncTest("jQuery.fn.load( String, Function ) - dataFilter in ajaxSettings",
		2,
		function(assert) {
			jQuery.ajaxSetup({
				dataFilter: function() {
			...
			jQuery("<div/>").load(url("data/name.html"),
	...
	QUnit.asyncTest("jQuery.fn.load( String, Object, Function )",
		2,
		function(assert) {
	...
	QUnit.test("jQuery.fn.load( String, String, Function )",
		2,
		function(assert) {
			var done = assert.async();
	...
	QUnit.test("jQuery.fn.load() - callbacks get the correct parameters",
		8,
		function(assert) {
			var completeArgs = {};
			var done = assert.async();
				...
				jQuery.map([
					...
					function(options) {
						return jQuery.Deferred(function(defer) {
							jQuery("#foo").load(options.url,
								function() {
									var args = arguments;
									assert.strictEqual(completeArgs[options.url].length,
										args.length,
										"same number of arguments (" + options.type + ")");
									jQuery.each(completeArgs[options.url],
										function(i, value) {
	...
	QUnit.test("#2046 - jQuery.fn.load( String, Function ) with ajaxSetup on dataType json",
		1,
		function(assert) {
			var done = assert.async();
	...
	QUnit.test("#10524 - jQuery.fn.load() - data specified in ajaxSettings is merged in",
		1,
		function(assert) {
			var done = assert.async();
...
// //----------- jQuery.post()
	...
	QUnit.test("jQuery.post( String, Hash, Function ) - simple with xml",
		4,
		function(assert) {
			var done = assert.async();
			...
			jQuery.when(
				...
				jQuery.post(url("data/name.php?xml=5-2"),
					{},
					function(xml) {
						jQuery("math", xml).each(function() {
							assert.strictEqual(jQuery("calculation", this).text(), "5-2", "Check for XML");
	...
	QUnit.test("jQuery[get|post]( options ) - simple with xml",
		2,
		function(assert) {
			var done = assert.async();
