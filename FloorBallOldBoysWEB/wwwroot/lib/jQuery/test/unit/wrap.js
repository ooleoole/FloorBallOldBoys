...
// See test/unit/manipulation.js for explanation about these 2 functions
	...
	function testWrap(val, assert) {
		...
		assert.equal(
			cacheLength,
			0,
			"No memory leak in jQuery.cache (bug #7165)"
		);
		...
		assert.equal(
			j.length,
			1,
			"There should only be one element (no cloning)."
		);
		assert.equal(
			j[0].parentNode.nodeName.toUpperCase(),
			"P",
		...
		result = jQuery("<div></div>").on("click",
			function() {
				assert.ok(
					true,
					"Event triggered."
	...
	QUnit.test("wrap(String|Element)",
		function(assert) {
	...
	QUnit.test("wrap(Function) with index (#10177)",
		function(assert) {
			var expectedIndex = 0,
	...
	QUnit.test("wrap(String) consecutive elements (#10177)",
		function(assert) {
	...
	QUnit.test("wrapAll(Function) check execution characteristics",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("wrapAll(Element)",
		function(assert) {
			...
			assert.equal(
				jQuery("#first").parent()[0].parentNode,
				p,
				"Correct Parent"
			);
		});

	QUnit.test("wrapInner(String)",
		function(assert) {
	...
	QUnit.test("wrapInner(Element)",
		function(assert) {
			...
			assert.ok(
				jQuery("#first").children().is("#empty"),
				"Verify Right Element"
			);
			assert.equal(
	...
	QUnit.test("wrapInner(Function) returns String",
		function(assert) {
	...
	QUnit.test("wrapInner(Function) returns Element",
		function(assert) {
	...
	QUnit.test("jQuery(<tag>) & wrap[Inner/All]() handle unknown elems (#10667)",
		function(assert) {
	...
	QUnit.test("wrapping scripts (#10470)",
		function(assert) {
