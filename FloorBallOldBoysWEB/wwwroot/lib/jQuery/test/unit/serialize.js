...
QUnit.test("jQuery.param()",
	function(assert) {
		...
		params = {
			"a": [
				0, [1, 2], [3, [4, 5], [6]],
				{ "b": [7, [8, 9], [{ "c": 10, "d": 11 }], [[12]], [[[13]]], { "e": { "f": { "g": [14, [15]] } } }, 16] }, 17
			]
		};
		assert.equal(decodeURIComponent(jQuery.param(params)),
		...
		assert.equal(
			decodeURIComponent(jQuery.param({
				"a": [1, 2, 3],
				"b[]": [4, 5, 6],
				"c[d]": [7, 8, 9],
				"e": { "f": [10], "g": [11, 12], "h": 13 }
		...
		// #7945
		assert.equal(jQuery.param({ "jquery": "1.4.2" }),
			"jquery=1.4.2",
...
QUnit.test("jQuery.param() not affected by ajaxSettings",
	function(assert) {
		assert.expect(1);
...
QUnit.test("jQuery.param() Constructed prop values",
	function(assert) {
		assert.expect(4);
