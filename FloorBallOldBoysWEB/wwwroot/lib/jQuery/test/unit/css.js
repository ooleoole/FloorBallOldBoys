if (jQuery.css) {
	...
	QUnit.test("css(String|Hash)",
		function(assert) {
			...
			div2 = jQuery(
					"<div style='display:none;'><input type='text' style='height:20px;'/><textarea style='height:20px;'/><div style='height:20px;'></div></div>")
				.appendTo("body");
			...
			assert.equal(jQuery("#floatTest").css("float"),
				"right",
			...
			jQuery.each("0,0.25,0.5,0.75,1".split(","),
				function(i, n) {
			...
			assert.strictEqual(div.css("z-index"),
				"1000",
				"Make sure that a string z-index is returned from css('z-index') (#14432).");
		});

	QUnit.test("css() explicit and relative values",
		function(assert) {
			assert.expect(29);
	...
	QUnit.test("css() non-px relative values (gh-1711)",
		function(assert) {
			assert.expect(17);
	...
	QUnit.test("css(String, Object)",
		function(assert) {
			...
			assert.strictEqual(jQuery("#foo").css("line-height"),
				"21px",
				"Set font shorthand property (#14759)");
		});

	QUnit.test("css(String, Object) with negative values",
		function(assert) {
			assert.expect(4);
	...
	QUnit.test("css(String, Function)",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("css(String, Function) with incoming value",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("css(Object) where values are Functions",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("css(Object) where values are Functions with incoming values",
		function(assert) {
			assert.expect(3);
...
// .show(), .hide(), can be excluded from the build
	if (jQuery.fn.show && jQuery.fn.hide) {
		...
		QUnit.test("show/hide detached nodes",
			function(assert) {
				assert.expect(19);
		...
		QUnit.test("hide hidden elements (bug #7141)",
			function(assert) {
				assert.expect(3);
		...
		QUnit.test("show() after hide() should always set display to initial value (#14750)",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("show/hide 3.0, default display",
			function(assert) {
		...
		QUnit.test("show/hide 3.0, default body display",
			function(assert) {
		...
		QUnit.test("show/hide 3.0, cascade display",
			function(assert) {
		...
		QUnit.test("show/hide 3.0, inline display",
			function(assert) {
		...
		QUnit.test("show/hide 3.0, cascade hidden",
			function(assert) {
		...
		QUnit.test("show/hide 3.0, inline hidden",
			function(assert) {
	...
	QUnit[jQuery.find.compile && jQuery.fn.toggle ? "test" : "skip"]("toggle()",
		function(assert) {
			assert.expect(9);
			var div,
				oldHide,
	...
	QUnit[jQuery.find.compile && jQuery.fn.toggle ? "test" : "skip"]("detached toggle()",
		function(assert) {
			assert.expect(6);
			...
			assert.equal(cascadeHiddenDetached[0].style.display,
				"none",
				"detached, cascade-hidden element");
			assert.equal(cascadeHiddenDetached[1].style.display,
				"none",
				"cascade-hidden element in detached tree");
		});

	QUnit.test("jQuery.css(elem, 'height') doesn't clear radio buttons (bug #1095)",
		function(assert) {
			assert.expect(4);
	...
	QUnit.test("internal ref to elem.runtimeStyle (bug #7608)",
		function(assert) {
			assert.expect(1);
			var result = true;
	...
	QUnit.test("computed margins (trac-3333; gh-2237)",
		function(assert) {
			assert.expect(2);
			...
			assert.equal($div.css("marginRight"),
				"0px",
	...
	QUnit.test("box model properties incorrectly returning % instead of px, see #10639 and #12088",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("jQuery.cssProps behavior, (bug #8402)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("widows & orphans #8936",
		function(assert) {
	...
	QUnit.test("can't get css for disconnected in IE<9, see #10254 and #8388",
		function(assert) {
			assert.expect(2);
			var span, div;
	...
	QUnit.test("Ensure styles are retrieving from parsed html on document fragments",
		function(assert) {
			assert.expect(1);
	...
	QUnit.test("can't get background-position in IE<9, see #10796",
		function(assert) {
	...
	if (jQuery.fn.offset) {
		QUnit.test("percentage properties for left and top should be transformed to pixels, see #9505",
			function(assert) {
				assert.expect(2);
				var parent = jQuery(
	...
	QUnit.test("Do not append px (#9548, #12990, #2792)",
		function(assert) {
			assert.expect(3);
	...
	QUnit.test("css('width') and css('height') should respect box-sizing, see #11004",
		function(assert) {
			assert.expect(4);
	...
	(function() {
		...
		QUnit.test("css('width') and css('height') should return fractional values for nodes in the document",
			function(assert) {
				...
				assert.equal(Number(el.css("width").replace(/px$/, "")).toFixed(1),
					"33.3",
					"css('width') should return fractional values");
				assert.equal(Number(el.css("height").replace(/px$/, "")).toFixed(1),
					"88.8",
					"css('height') should return fractional values");
			});

		QUnit.test("css('width') and css('height') should return fractional values for disconnected nodes",
			function(assert) {
				if (!supportsFractionalGBCR) {
					assert.expect(1);
					assert.ok(true, "This browser doesn't support fractional values in getBoundingClientRect()");
	...
	QUnit.test("certain css values of 'normal' should be convertable to a number, see #8627",
		function(assert) {
			assert.expect(3);

			var el = jQuery("<div style='letter-spacing:normal;font-weight:normal;'>test</div>").appendTo("#qunit-fixture");

			assert.ok(jQuery.isNumeric(parseFloat(el.css("letterSpacing"))),
				"css('letterSpacing') not convertable to number, see #8627");
			assert.ok(jQuery.isNumeric(parseFloat(el.css("fontWeight"))),
...
// Only run this test in IE9
	if (document.documentMode === 9) {
		QUnit.test(".css('filter') returns a string in IE9, see #12537",
			function(assert) {
				assert.expect(1);
	...
	QUnit.test("css opacity consistency across browsers (#12685)",
		function(assert) {
			assert.expect(3);
	...
	QUnit[jQuery.find.compile ? "test" : "skip"](":visible/:hidden selectors",
		function(assert) {
			assert.expect(17);
			...
			assert.equal(jQuery("#table td:visible").length,
				1,
	...
	QUnit.test("Keep the last style if the new one isn't recognized by the browser (#14836)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("Reset the style if set to an empty string",
		function(assert) {
			assert.expect(1);
			...
			assert.equal(el.css("position") || "static",
				"static",
...
// but it's the best that we can do.
	(function() {
		...
		if (exist) {
			QUnit.test("Don't append px to CSS \"order\" value (#14049)",
				function(assert) {
					assert.expect(1);
	...
	QUnit.test("Do not throw on frame elements from css method (#15098)",
		function(assert) {
			assert.expect(1);
	...
	(function() {
		...
		function resetCssPropsFor(name) {
			...
			jQuery.each(vendorPrefixes,
				function(index, prefix) {
					delete jQuery.cssProps[prefix + name[0].toUpperCase() + name.slice(1)];
				});
		}

		QUnit.test("Don't default to a cached previously used wrong prefixed name (gh-2015)",
			function(assert) {
	...
	(function() {
		...
		QUnit[supportsCssVars ? "test" : "skip"]("css(--customProperty)",
			function(assert) {
