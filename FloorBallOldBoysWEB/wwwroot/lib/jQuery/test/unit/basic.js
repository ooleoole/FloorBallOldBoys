...
QUnit.test("core",
	function(assert) {
		...
		assert.deepEqual(jQuery.map(["a", "b", "c"],
				function(v, k) {
					return k + v;
				}),
			["0a", "1b", "2c"],
		...
		assert.deepEqual(jQuery.grep([1, 2, 3],
				function(value) {
					return value % 2 !== 0;
				}),
			[1, 3],
