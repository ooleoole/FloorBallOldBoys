...
QUnit.test("jQuery.propFix integrity test",
	function(assert) {
		assert.expect(1);
...
QUnit.test("attr(String)",
	function(assert) {
		...
		extras = jQuery("<input id='id' name='id'/><input id='name' name='name'/><input id='target' name='target'/>")
			.appendTo("#testForm");
		assert.equal(jQuery("#form").attr("action", "newformaction").attr("action"),
			"newformaction",
			"Check that action attribute was changed");
		assert.equal(jQuery("#testForm").attr("target"),
			undefined,
			"Retrieving target does not equal the input with name=target");
		assert.equal(jQuery("#testForm").attr("target", "newTarget").attr("target"),
			"newTarget",
			"Set target successfully on a form");
		assert.equal(jQuery("#testForm").removeAttr("id").attr("id"),
			undefined,
		...
		assert.equal(jQuery("#tAnchor5").prop("href"),
		...
		assert.equal(jQuery(option).prop("selected"),
			true,
		...
		assert.equal($button.attr("value", "foobar").attr("value"),
			"foobar",
		...
		assert.equal(jQuery("#table").attr("test:attrib"),
			undefined,
			"Retrieving a non-existent attribute on a table with a colon does not throw an error.");
		assert.equal(jQuery("#table").attr("test:attrib", "foobar").attr("test:attrib"),
			"foobar",
		...
		assert.ok(jQuery("<div/>").attr("doesntexist") === undefined,
			"Make sure undefined is returned when no attribute is found.");
		assert.ok(jQuery("<div/>").attr("title") === undefined,
			"Make sure undefined is returned when no attribute is found.");
		...
		assert.strictEqual(jQuery("<select><option value='property'></option></select>").attr("value"),
			undefined,
...
QUnit.test("attr(String) on cloned elements, #9646",
	function(assert) {
		assert.expect(4);
		...
		assert.strictEqual(input.clone(true).attr("name", "test")[0].name,
			"test",
		...
		assert.strictEqual(div.clone(true).attr("id", "test")[0].id,
			"test",
		...
		assert.strictEqual(input.clone(true).attr("value", "test")[0].value,
			"test",
			"Value attribute should be changed on cloned element");

		assert.strictEqual(input.clone(true).attr("value", 42)[0].value,
			"42",
			"Value attribute should be changed on cloned element");
	});

QUnit.test("attr(String) in XML Files",
	function(assert) {
		assert.expect(3);
		...
		assert.equal(jQuery("location", xml).attr("checked"),
			"different",
...
QUnit.test("attr(String, Object)",
	function(assert) {
		...
		assert.equal($text.attr("required", false).attr("required"),
			undefined,
		...
		assert.equal($text.attr("aria-disabled", false).attr("aria-disabled"),
			"false",
		...
		jQuery.each([commentNode, textNode, attributeNode],
			function(i, elem) {
				var $elem = jQuery(elem);
				$elem.attr("nonexisting", "foo");
				assert.strictEqual($elem.attr("nonexisting"),
					undefined,
					"attr(name, value) works correctly on comment and text nodes (bug #7500).");
			});

		jQuery.each([window, document, obj, "#firstp"],
			function(i, elem) {
				...
				assert.equal($elem.attr("nonexisting", "foo").attr("nonexisting"),
					"foo",
		...
		table = jQuery("#table")
			.append("<tr><td>cell</td></tr><tr><td>cell</td><td>cell</td></tr><tr><td>cell</td><td>cell</td></tr>");
		...
		assert.equal(typeof jQuery("#name").attr("maxlength", undefined),
			"object",
			".attr('attribute', undefined) is chainable (#5571)");
		assert.equal(jQuery("#name").attr("maxlength", undefined).attr("maxlength"),
			"5",
			".attr('attribute', undefined) does not change value (#5571)");
		assert.equal(jQuery("#name").attr("nonexisting", undefined).attr("nonexisting"),
			undefined,
			".attr('attribute', undefined) does not create attribute (#5571)");
	});

QUnit.test("attr(non-ASCII)",
	function(assert) {
		assert.expect(2);
...
QUnit.test("attr - extending the boolean attrHandle",
	function(assert) {
		assert.expect(1);
		var called = false,
...
QUnit.test("attr(String, Object) - Loaded via XML document",
	function(assert) {
		assert.expect(2);
		var xml = createDashboardXML(),
			titles = [];
...
QUnit.test("attr(String, Object) - Loaded via XML fragment",
	function(assert) {
		assert.expect(2);
...
QUnit.test("attr('tabindex')",
	function(assert) {
		...
		// anchor without href
		assert.equal(jQuery("#linkWithNoHrefWithNoTabIndex").attr("tabindex"),
			undefined,
		...
		assert.equal(jQuery("#linkWithNoHrefWithNegativeTabIndex").attr("tabindex"),
			"-1",
			"anchor without href, no tabindex set");
	});

QUnit.test("attr('tabindex', value)",
	function(assert) {
		assert.expect(9);
...
QUnit.test("removeAttr(String)",
	function(assert) {
		...
		assert.equal(jQuery("#foo").attr("style", "position:absolute;").removeAttr("style").attr("style"),
			undefined,
			"Check removing style attribute");
		assert.equal(jQuery("#form").attr("style", "position:absolute;").removeAttr("style").attr("style"),
			undefined,
			"Check removing style attribute on a form");
		assert.equal(jQuery("<div style='position: absolute'></div>").appendTo("#foo").removeAttr("style").prop("style")
			.cssText,
			"",
			"Check removing style attribute (#9699 Webkit)");
		assert.equal(jQuery("#fx-test-group").attr("height", "3px").removeAttr("height").get(0).style.height,
			"1px",
			"Removing height attribute has no effect on height set with style attribute");

		jQuery("#check1").removeAttr("checked").prop("checked", true).removeAttr("checked");
		assert.equal(document.getElementById("check1").checked,
			true,
		...
		assert.equal(jQuery("#option2d").attr("selected"),
			"selected",
...
QUnit.test("removeAttr(String) in XML",
	function(assert) {
		assert.expect(7);
		var xml = createDashboardXML(),
...
QUnit.test("removeAttr(Multi String, variable space width)",
	function(assert) {
		assert.expect(8);
...
QUnit.test("removeAttr(Multi String, non-HTML whitespace is valid in attribute names (gh-3003)",
	function(assert) {
		assert.expect(8);

		var div = jQuery("<div id='a' data-='b' title='c' rel='d'></div>");
		var tests = {
			id: "a",
			"data-\xA0": "b",
			title: "c",
...
QUnit.test("prop(String, Object)",
	function(assert) {

		assert.expect(17);

		assert.equal(jQuery("#text1").prop("value"), "Test", "Check for value attribute");
		assert.equal(jQuery("#text1").prop("value", "Test2").prop("defaultValue"),
			"Test",
...
QUnit.test("prop(String, Object) on null/undefined",
	function(assert) {
		...
		textNode = document.createTextNode("some text");
		obj = {};
		jQuery.each([document, attributeNode, commentNode, textNode, obj, "#firstp"],
			function(i, ele) {
				assert.strictEqual(jQuery(ele).prop("nonexisting"),
					undefined,
					"prop works correctly for non existing attributes (bug #7500).");
			});

		obj = {};
		jQuery.each([document, obj],
			function(i, ele) {
				var $ele = jQuery(ele);
				$ele.prop("nonexisting", "foo");
				assert.equal($ele.prop("nonexisting"),
					"foo",
...
QUnit.test("prop('tabindex')",
	function(assert) {
		...
		assert.equal(jQuery("#linkWithNoHrefWithNegativeTabIndex").prop("tabindex"),
			-1,
			"anchor without href, no tabindex set");
	});

QUnit.test("image.prop( 'tabIndex' )",
	function(assert) {
		assert.expect(1);
...
QUnit.test("prop('tabindex', value)",
	function(assert) {
		assert.expect(10);
...
QUnit.test("option.prop('selected', true) affects select.selectedIndex (gh-2732)",
	function(assert) {
		assert.expect(2);
	...
	function(assert) {
		...
		jQuery.each([document, obj],
			function(i, ele) {
				...
				assert.strictEqual(ele["nonexisting"],
					undefined,
...
QUnit.test("val() after modification",
	function(assert) {
...
QUnit.test("val()",
	function(assert) {
		...
		assert.strictEqual(
			jQuery("<select name='select12584' id='select12584'><option value='1' disabled='disabled'>1</option></select>")
			.val(),
			null,
...
QUnit.test("val() with non-matching values on dropdown list",
	function(assert) {
		assert.expect(3);
		...
		var select6 =
			jQuery("<select multiple id=\"select6\"><option value=\"1\">A</option><option value=\"2\">B</option></select>")
				.appendTo("#form");
...
if ("value" in document.createElement("meter") &&
	...
	QUnit.test("val() respects numbers without exception (Bug #9319)",
		function(assert) {
...
QUnit.test("val(Array of Numbers) (Bug #7123)",
	function(assert) {
		assert.expect(4);
		jQuery("#form")
			.append(
...
QUnit.test("val(Function) with incoming value",
	function(assert) {
		assert.expect(10);
...
QUnit.test("val(select) after form.reset() (Bug #2551)",
	function(assert) {
		assert.expect(3);
...
QUnit.test("select.val(space characters) (gh-2978)",
	function(assert) {
		assert.expect(37);
		...
		jQuery.each(spaces,
			function(key, obj) {
				...
				if (/^\\u/.test(key)) {
					...
					assert.equal($select.val(),
						val + "text",
						"Value with non-HTML space character at beginning is not stripped (" + key + ") selected (" + key + "text)");
					$select.val("te" + val + "xt");
					assert.equal($select.val(),
						"te" + val + "xt",
						"Value with non-space whitespace character (" + key + ") in the middle selected (text)");
					$select.val("text" + val);
					assert.equal($select.val(),
						"text" + val,
						"Value with non-HTML space character at end is not stripped (" + key + ") selected (text" + key + ")");
				} else {
					$select.val("text");
					assert.equal($select.val(),
						"text",
						"Value with HTML space character at beginning or end is stripped (" + key + ") selected (text)");
					$select.val("te xt");
...
QUnit.test("addClass(String)",
	function(assert) {
		testAddClass(bareObj, assert);
	});

QUnit.test("addClass(Function)",
	function(assert) {
		testAddClass(functionReturningObj, assert);
	});

QUnit.test("addClass(Function) with incoming value",
	function(assert) {
		assert.expect(52);
		var pass,
			i,
...
QUnit.test("removeClass(String) - simple",
	function(assert) {
		testRemoveClass(bareObj, assert);
	});

QUnit.test("removeClass(Function) - simple",
	function(assert) {
		testRemoveClass(functionReturningObj, assert);
	});

QUnit.test("removeClass(Function) with incoming value",
	function(assert) {
		assert.expect(52);
...
QUnit.test("removeClass() removes duplicates",
	function(assert) {
		assert.expect(1);
...
QUnit.test("removeClass(undefined) is a no-op",
	function(assert) {
		assert.expect(1);
...
QUnit.test("toggleClass(String|boolean|undefined[, boolean])",
	function(assert) {
		testToggleClass(bareObj, assert);
	});

QUnit.test("toggleClass(Function[, boolean])",
	function(assert) {
		testToggleClass(functionReturningObj, assert);
	});

QUnit.test("toggleClass(Function[, boolean]) with incoming value",
	function(assert) {
		assert.expect(14);
...
QUnit.test("addClass, removeClass, hasClass",
	function(assert) {
		assert.expect(17);
...
QUnit.test("addClass, removeClass, hasClass on many elements",
	function(assert) {
		assert.expect(19);
...
QUnit.test("addClass, removeClass, hasClass on elements with classes with non-HTML whitespace (gh-3072, gh-3003)",
	function(assert) {
		assert.expect(9);
...
QUnit.test("contents().hasClass() returns correct values",
	function(assert) {
		assert.expect(2);
...
QUnit.test("hasClass correctly interprets non-space separators (#13835)",
	function(assert) {
		assert.expect(4);
...
QUnit.test("coords returns correct values in IE6/IE7, see #10828",
	function(assert) {
		assert.expect(1);
...
QUnit.test("should not throw at $(option).val() (#14686)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("option value not trimmed when setting via parent select",
	function(assert) {
		assert.expect(1);
		assert.equal(jQuery("<select><option> 2</option></select>").val("2").val(), "2");
	});

QUnit.test("Insignificant white space returned for $(option).val() (#14858, gh-2978)",
	function(assert) {
		assert.expect(16);
...
QUnit.test("SVG class manipulation (gh-2199)",
	function(assert) {
		assert.expect(12);
...
QUnit.test("non-lowercase boolean attribute getters should not crash",
	function(assert) {
		assert.expect(3);
		...
		jQuery.each({
			...
			function(lowercased, original) {
				try {
					assert.strictEqual(elem.attr(original),
						lowercased,
