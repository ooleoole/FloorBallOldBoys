...
(function() {
	...
	QUnit.test("jQuery.when(jQuery.ready)",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("Promise.resolve(jQuery.ready)",
		function(assert) {
			assert.expect(2);
			var done = jQuery.map(new Array(2), function() { return assert.async(); });
	...
	QUnit.test("Error in ready callback does not halt all future executions (gh-1823)",
		function(assert) {
			assert.expect(1);
			var done = assert.async();
