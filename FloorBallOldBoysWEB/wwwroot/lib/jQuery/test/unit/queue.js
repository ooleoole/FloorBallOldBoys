...
QUnit.test("queue() with other types",
	function(assert) {
		assert.expect(14);
		...
		assert.equal($div.queue("foo", undefined).queue("foo").length,
			4,
...
QUnit.test("queue(name) passes in the next item in the queue as a parameter",
	function(assert) {
		assert.expect(2);
...
QUnit.test("queue() passes in the next item in the queue as a parameter to fx queues",
	function(assert) {
		assert.expect(3);
		QUnit.stop();
...
QUnit.test("callbacks keep their place in the queue",
	function(assert) {
		assert.expect(5);
		QUnit.stop();
		var div = jQuery("<div>"),
...
QUnit.test("jQuery.queue should return array while manipulating the queue",
	function(assert) {
		assert.expect(1);

		var div = document.createElement("div");

		assert.ok(Array.isArray(jQuery.queue(div, "fx", jQuery.noop)),
...
QUnit.test("clearQueue(name) clears the queue",
	function(assert) {
		assert.expect(2);
...
QUnit.test("clearQueue() clears the fx queue",
	function(assert) {
		assert.expect(1);
...
QUnit.asyncTest("fn.promise() - called when fx queue is empty",
	3,
	function(assert) {
...
QUnit.asyncTest("fn.promise( \"queue\" ) - called whenever last queue function is dequeued",
	5,
	function(assert) {
		var foo = jQuery("#foo"),
			test;
...
if (jQuery.fn.animate) {
	...
	QUnit.asyncTest("fn.promise( \"queue\" ) - waits for animation to complete before resolving",
		2,
		function(assert) {
			var foo = jQuery("#foo"),
				test = 1;
...
if (jQuery.fn.stop) {
	QUnit.test("delay() can be stopped",
		function(assert) {
			assert.expect(3);
			QUnit.stop();
	...
	QUnit.asyncTest("queue stop hooks",
		2,
		function(assert) {
			var foo = jQuery("#foo");
