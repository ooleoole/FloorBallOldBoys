...
QUnit.test("null or undefined handler",
	function(assert) {
		assert.expect(4);
...
QUnit.test("on() with non-null,defined data",
	function(assert) {
...
QUnit.test("Handler changes and .trigger() order",
	function(assert) {
		assert.expect(1);
...
QUnit.test("on(), with data, trigger with data",
	function(assert) {
		assert.expect(4);
		var handler = function(event, data) {
...
QUnit.test("on(), multiple events at once",
	function(assert) {
		assert.expect(2);
		var handler,
			clickCounter = 0,
...
QUnit.test("on(), five events at once",
	function(assert) {
		assert.expect(1);
...
QUnit.test("on(), multiple events at once and namespaces",
	function(assert) {
		assert.expect(7);
		...
		div = jQuery("<div/>").on("click mouseover",
			obj,
			function(e) {
		...
		div = jQuery("<div/>").on("focusin.a focusout.b",
			function(e) {
				assert.equal(e.type, cur, "Verify right multi event was fired.");
			});
...
QUnit.test("on(), namespace with special add",
	function(assert) {
		assert.expect(27);

		var i = 0,
			div = jQuery("<div/>").appendTo("#qunit-fixture").on("test",
				function() {
...
QUnit.test("on/off(Object), on/off(Object, String)",
	function(assert) {
		assert.expect(6);
...
QUnit.test("on immediate propagation",
	function(assert) {
		assert.expect(2);
...
QUnit.test("on bubbling, isDefaultPrevented, stopImmediatePropagation",
	function(assert) {
		assert.expect(3);
...
QUnit.test("on(), namespaced events, cloned events",
	function(assert) {
		assert.expect(18);

		var firstp = jQuery("#firstp");

		firstp.on("custom.test",
			function() {
		...
		jQuery("#nonnodes").contents().on("tester",
			function() {
...
QUnit.test("on(), multi-namespaced events",
	function(assert) {
		assert.expect(6);
...
QUnit.test("namespace-only event binding is a no-op",
	function(assert) {
		assert.expect(2);
...
QUnit.test("Empty namespace is ignored",
	function(assert) {
		assert.expect(1);
...
QUnit.test("on(), with same function",
	function(assert) {
		assert.expect(2);
...
QUnit.test("on(), make sure order is maintained",
	function(assert) {
		assert.expect(1);

		var elem = jQuery("#firstp"), log = [], check = [];

		jQuery.each(new Array(100),
			function(i) {
				elem.on("click",
					function() {
						log.push(i);
...
QUnit.test("on(), with different this object",
	function(assert) {
		assert.expect(4);
...
QUnit.test("on(name, false), off(name, false)",
	function(assert) {
		assert.expect(3);
...
QUnit.test("on(name, selector, false), off(name, selector, false)",
	function(assert) {
		assert.expect(3);
...
QUnit.test("on()/trigger()/off() on plain object",
	function(assert) {
		assert.expect(7);
...
if (jQuery.fn.hover) {
	QUnit.test("hover() mouseenter mouseleave",
		function(assert) {
			assert.expect(1);
...
QUnit.test("mouseover triggers mouseenter",
	function(assert) {
		assert.expect(1);
...
QUnit.test("pointerover triggers pointerenter",
	function(assert) {
		assert.expect(1);

		var count = 0,
			elem = jQuery("<a/>");
		elem.on("pointerenter",
			function() {
				count++;
			});
...
QUnit.test("withinElement implemented with jQuery.contains()",
	function(assert) {
		...
		jQuery("#jc-outer").on("mouseenter mouseleave",
			function(event) {
...
QUnit.test("mouseenter, mouseleave don't catch exceptions",
	function(assert) {
		assert.expect(2);

		var elem = jQuery("#firstp").on("mouseenter mouseleave",
			function() {
				throw "an Exception";
			});
...
QUnit.test("trigger(type, [data], [fn])",
	function(assert) {
		assert.expect(16);
...
QUnit.test("change event bubbles on copied forms (#11796)",
	function(assert) {
		assert.expect(3);
...
QUnit.test("trigger(eventObject, [data], [fn])",
	function(assert) {
		assert.expect(28);
...
QUnit.test(".trigger() bubbling on disconnected elements (#10489)",
	function(assert) {
		assert.expect(2);
...
QUnit.test(".trigger() doesn't bubble load event (#10717)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("Delegated events in SVG (#10791; #13180)",
	function(assert) {
		assert.expect(2);
...
QUnit.test("Delegated events with malformed selectors (gh-3071)",
	function(assert) {
		assert.expect(3);
...
QUnit.test("Delegated events in forms (#10844; #11145; #8165; #11382, #11764)",
	function(assert) {
		assert.expect(5);
...
QUnit.test("Submit event can be stopped (#11049)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("jQuery.Event( type, props )",
	function(assert) {
		...
		assert.strictEqual(jQuery.isPlainObject(event),
			false,
...
QUnit.test("jQuery.Event properties",
	function(assert) {
		assert.expect(12);
...
QUnit.test("jQuery.off using dispatched jQuery.Event",
	function(assert) {
		assert.expect(1);
...
QUnit[jQuery.find.compile ? "test" : "skip"]("delegated event with delegateTarget-relative selector",
	function(assert) {
		assert.expect(3);
		var markup =
			jQuery(
...
QUnit.test("delegated event with selector matching Object.prototype property (#13203)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("delegated event with intermediate DOM manipulation (#13208)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("ignore comment nodes in event delegation (gh-2055)",
	function(assert) {
		assert.expect(1);
		...
		var $foo = jQuery("#foo").on("DOMNodeInserted",
			    "[id]",
			    function() {
...
QUnit.test("stopPropagation() stops directly-bound events on delegated target",
	function(assert) {
		assert.expect(1);
...
QUnit.test("off all bound delegated events",
	function(assert) {
		assert.expect(2);
...
QUnit.test("on with multiple delegated events",
	function(assert) {
		assert.expect(1);
...
QUnit.test("delegated on with change",
	function(assert) {
		assert.expect(8);
...
QUnit.test("delegated on with submit",
	function(assert) {
		assert.expect(2);
...
QUnit.test("delegated off() with only namespaces",
	function(assert) {
		assert.expect(2);
...
QUnit.test("Non DOM element events",
	function(assert) {
		assert.expect(1);

		var o = {};

		jQuery(o).on("nonelementobj",
			function() {
...
QUnit.test("inline handler returning false stops default",
	function(assert) {
		assert.expect(1);
...
QUnit.test("focusin bubbles",
	function(assert) {
		...
		jQuery("body").on("focusin.focusinBubblesTest",
			function() {
				assert.equal(1, order++, "focusin on the body second");
			});

		input.on("focusin.focusinBubblesTest",
			function() {
...
QUnit.test("custom events with colons (#3533, #8272)",
	function(assert) {
		assert.expect(1);
...
QUnit.test("special on name mapping",
	function(assert) {
		assert.expect(7);
...
QUnit.test(".on and .off, selective mixed removal (#10705)",
	function(assert) {
		assert.expect(7);
...
QUnit.test(".on( event-map, null-selector, data ) #11130",
	function(assert) {
...
QUnit.test("clone() delegated events (#11076)",
	function(assert) {
		assert.expect(3);
...
QUnit.test("checkbox state (#3827)",
	function(assert) {
		assert.expect(9);
...
QUnit.test("hover event no longer special since 1.9",
	function(assert) {
		assert.expect(1);
...
QUnit.test("event object properties on natively-triggered event",
	function(assert) {
		assert.expect(3);
...
QUnit.test("drag/drop events copy mouse-related event properties (gh-1925, gh-2009)",
	function(assert) {
		assert.expect(4);
...
QUnit.test("focusin using non-element targets",
	function(assert) {
		assert.expect(2);
...
testIframe(
	...
	function(assert, framejQuery, frameWin, frameDoc) {
		...
		jQuery("body").on("focusin.iframeTest",
			function() {
...
QUnit.test("change handler should be detached from element",
	function(assert) {
		assert.expect(2);
...
QUnit.test("trigger click on checkbox, fires change event",
	function(assert) {
		assert.expect(1);
...
QUnit.test("Namespace preserved when passed an Event (#12739)",
	function(assert) {
		assert.expect(4);
...
QUnit.test("make sure events cloned correctly",
	function(assert) {
		assert.expect(18);
...
QUnit.test("String.prototype.namespace does not cause trigger() to throw (#13360)",
	function(assert) {
		assert.expect(1);
		var errored = false;
...
QUnit.test("Inline event result is returned (#13993)",
	function(assert) {
		assert.expect(1);
...
QUnit.test(".off() removes the expando when there's no more data",
	function(assert) {
		assert.expect(2);
...
QUnit.test("jQuery.Event( src ) does not require a target property",
	function(assert) {
		assert.expect(2);
...
QUnit.test("preventDefault() on focusin does not throw exception",
	function(assert) {
		assert.expect(1);
		...
		input
			.on("focusin",
				function(event) {
					...
					assert.strictEqual(exceptionCaught,
						undefined,
...
QUnit.test("Donor event interference",
	function(assert) {
		assert.expect(10);
...
QUnit.test("originalEvent property for Chrome, Safari, Fx & Edge of simulated event",
	function(assert) {
		var userAgent = window.navigator.userAgent;
...
QUnit.test("trigger('click') on radio passes extra params",
	function(assert) {
		assert.expect(1);
...
QUnit[jQuery.fn.click ? "test" : "skip"]("trigger() shortcuts",
	function(assert) {
		assert.expect(5);
...
QUnit[jQuery.fn.click ? "test" : "skip"]("Event aliases",
	function(assert) {
...
QUnit.test("VML with special event handlers (trac-7071)",
	function(assert) {
		assert.expect(1);
...
if (!(/firefox/i.test(window.navigator.userAgent))) {
	QUnit.test("Check order of focusin/focusout events",
		function(assert) {
			assert.expect(2);
	...
	QUnit.test("focus-blur order (#12868)",
		function(assert) {
			assert.expect(5);
